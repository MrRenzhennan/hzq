...
var DisCusRegisterInfo = function () {
    ...
    var todelete = function () {
        return {
            f : function (val,row) {
                return '<a onclick="DisCusRegisterInfo.delArchive4Nonrsdt(\''+ row["ctmArchiveId"]+"','"+gasBllNonrsdtDctId + "')\">&nbsp删除</a>";
                //return '<a onclick="delArchive4Nonrsdt('+ row["ctmArchiveId"]+','+gasBllNonrsdtDctId + ')">&nbsp删除</a>';
                //return "<a href='charging/application_add.html?nonRsdtDctId=" +  row["nonRsdtDctId"] + "\'>" + "&nbsp详情&变更"+ "</a>";
            }
        }
    }();
