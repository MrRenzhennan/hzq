...
var correctApprovalStep = function () {
    ...
    return {
        ...
        initPage: function () {
            var correctFlow =  Restful.findNQ(hzq_rest + "gasbllcorrectflow/"+ correctFlowId);
            console.log(correctFlow)
            if(correctFlow.correctType == "2"){
                $('#correcttype').val(correctFlow.correctType).trigger("change");
                $("#correctgass").css("display","none")
            }else{
                $('#correcttype').val("1").trigger("change");
                $('#correctGas').val(correctFlow.correctGas);
            }
            $('#correctMon').val(correctFlow.correctMon);
            $('#correctReason').val(correctFlow.correctReason);
            $('#remark').val(correctFlow.remark);
            console.log("result"+correctFlowId);
            if(correctFlow.correctFlowId ==undefined){
                ...
                $.ajax(
                    {
                        ...
                        type: 'get',
                        url: hzq_rest + 'gasctmarchive/' + correctFlow.ctmArchiveId,
                        dataType: 'json',
                        data: "",
                        success: function (data) {
                            ...
                            $('#userTel1').val(jsonData.userTel1);
