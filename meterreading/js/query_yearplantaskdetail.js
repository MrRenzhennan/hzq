/**
 * Created by anne on 2017/12/25.
 * 年计划任务生成
 */
var loginarea = [];
var areaId = JSON.parse(localStorage.getItem("user_info")).area_id;
var detailreason ={"1":"抄表抄表月份为空","2":"抄表抄表例日为空","3":"非居民例日与月份数目不符","4":"抄表例日或抄表月份为空","5":"非居民只存在一个例日","6":"非居民例日可能存在错误","7":"未找到抄表本信息","8":"未找到例日","9":"未找到表户信息","10":"找到多条表户信息","11":"未找到房屋信息","12":"未找到房屋信息","13":"非居民抄表例日表缺失本客户","14":"非居民抄表例日中获取到两条或多条记录","15":"非居民抄表例日表缺失本客户"};
...
var MrdYearPlanDetailSearchAction= function () {
    //用户helper
...
//      }
//  }()
var mrdTaskStateFormat = function(){
		return {
			f:function(val){
				if(val =='1'){
					return "未开始";
...
//find_reason
    ...
    return {
        ...
        initHelper:function(){
        		//年计划错误原因
        		$.map(detailreason,function(key,val){
        			$('#find_reason').append('<option value="'+val+'">'+key+'</option>');
        		});
...
//var bd={
//	"cols":"t.book_id,br.book_code,br.countper_id,br.area_id,br.serviceper_id,t.created_num,t.mrd_task_status,t.created_time,t.plan_year,t.defeated_num",
//	"froms":"gas_mrd_book b left join gas_mrd_plan_job t on t.book_id=b.book_id ",
//	"wheres":"1=0  ",//and  b.status='1' and t.status='1'
//	"page":"false"
//}
var bd={
	"cols":"d.book_id,b.book_code,b.area_id,b.countper_id,b.serviceper_id,ar.customer_code,ar.customer_kind,ar.customer_name,d.reason ",
	"froms":"gas_mrd_plan_job_detail d left join gas_mrd_book b on b.book_id=d.book_id left join gas_ctm_archive ar on ar.customer_code=d.customer_code ",
	"wheres":"1=0",
	"page":"false"
};
            /* rf= RFTable.init(*/
...
//                      };
var whereinfo ='1=1 ';
if($('#find_areaId').val()){//供气区域额
	whereinfo +=" and b.area_id='"+$('#find_areaId').val()+"' " ;
}
if($('#find_countperId').val()){//核算员
	whereinfo +=" and b.countper_id='"+$('#find_countperId').val()+"' ";
}
if($('#find_serviceperId').val()){//客户服务员
	whereinfo +=" and b.serviceper_id='"+$('#find_serviceperId').val()+"' ";
}
if($('#find_bookCode').val()){//抄表本编号
	whereinfo +=" and b.book_code='"+$('#find_bookCode').val()+"' ";
}
if($('#find_mrdTaskStatus').val()){//年计划任务状态
	whereinfo +=" and d.mrd_task_status='"+$('#find_mrdTaskStatus').val()+"' ";
}
if($('#find_planYear').val()){//年计划计划年份
	whereinfo +=" and d.plan_year ='"+$('#find_planYear').val()+"' ";
}
if($('#find_reason').val()){
	whereinfo +=" and d.reason like '%"+detailreason[($('#find_reason').val())]+"%' ";
}
if($('#find_customerCode').val()){
	whereinfo +=" and ar.customer_code='"+$('#find_customerCode').val()+"' ";
}
if($('#find_customerAddress').val()){
	whereinfo +=" and ar.customer_address='"+$('#find_customerAddress').val()+"' ";
}
if($('#find_customerKind').val()){
	whereinfo +=" and ar.customer_kind ='"+$('#find_customerKind').val()+"' ";
}

//if($('#find_createdTime').val()){//年计划任务生成创建时间
//	whereinfo +='';
//}
//whereinfo +=" and br.area_id ='"+$('#find_areaId').val()+"' ";
//var bd={
//	"cols":"t.book_id,br.book_code,br.countper_id,br.area_id,br.serviceper_id,t.created_num,t.mrd_task_status,t.created_time,t.plan_year,t.defeated_num",
//	"froms":"gas_mrd_book b left join gas_mrd_plan_job t on t.book_id=b.book_id ",
//	"wheres":" b.status='1' and t.status='1'  "+whereinfo,//and 
//	"page":"false"
//}
var bd={
	"cols":"distinct d.book_id,b.book_code,b.area_id,b.countper_id,b.serviceper_id,ar.customer_code,ar.customer_kind,ar.customer_name,d.reason ",
	"froms":" gas_mrd_plan_job_detail d left join gas_mrd_book b on b.book_id=d.book_id left join gas_ctm_archive ar on ar.customer_code=d.customer_code ",
	"wheres":whereinfo+"",
	"page":true,
	"limit":50
