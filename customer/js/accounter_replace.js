...
var ScatteredServe = function(){
	return {
        ...
        reload: function () {
            ...
            checkPlan=Restful.getByID(hzq_rest+'gasmtrcheckplan',checkPlanDetail.checkPlanId);
...
$("#save_btn").click(function(){
	...
	if(changeMeter=="1"){
		...
		if(!checkmeterNo){
			bootbox.alert("<br><center>使用备用表请填写备用表编号</center></br>");
			return;
		}
	}
	...
	//判断是否需要计算计费读数
	var ischarge=true;
	if(changeMeter==2&&checkPlanDetail.checkState==3){
		ischarge=false;
	}
    ...
    if(changeMeter=="1"){
    	...
    	if(checkPlanDetail.checkState=='1'){
    		submitJson = {"sets":[
		        {"txid":"1","body":unboltJson4,"path":"/gasctmmeter/","method":"PUT"},
		        {"txid":"3","body":unboltJson3,"path":"/gasmrdmeterreading/","method":"POST"},
		        {"txid":"5","body":unboltJson6,"path":"/gascsrworkbill/","method":"PUT"}
	    	]}
    	}else if(checkPlanDetail.checkState=='3'){
    		submitJson = {"sets":[
		        {"txid":"1","body":unboltJson4,"path":"/gasctmmeter/","method":"PUT"},
		        {"txid":"2","body":newMeterReading,"path":"/gasmrdmeterreading/","method":"POST"},
		        {"txid":"5","body":unboltJson6,"path":"/gascsrworkbill/","method":"PUT"}
	...
	if(checkPlanDetail.checkState=='1'){
		checkPlanDetail.checkState="2";
	}else if(checkPlanDetail.checkState=='3'){
		var enCount=checkPlan.reservedField1;
		checkPlanDetail.checkState="4";
		checkPlan.reservedField1=parseInt(enCount)+1;
		checkPlan.modifiedTime=new Date(moment().format("YYYY-MM-DD HH:mm:ss")+"-00:00");
		submitJson.sets.push({"txid":"21","body":checkPlan,"path":"/gasmtrcheckplan/","method":"PUT"});
		//更新总公司年计划
		var xwQuery2 = RQLBuilder.and([
		    RQLBuilder.equal("checkYear",checkPlan.checkYear),RQLBuilder.equal("areaId","1")
		]).rql();
		var checkPlanData = Restful.findNQ(hzq_rest+"gasmtrcheckplan?query="+xwQuery2);
		if(checkPlanData&&checkPlanData.length>0){
			var checkPlanAll=checkPlanData[0];
			var enCountAll=checkPlanAll.reservedField1;
			checkPlanAll.reservedField1=parseInt(enCountAll)+1;
			checkPlanAll.modifiedTime=new Date(moment().format("YYYY-MM-DD HH:mm:ss")+"-00:00");
			submitJson.sets.push({"txid":"22","body":checkPlanAll,"path":"/gasmtrcheckplan/","method":"PUT"});
		}
		
		var checkMeter=Restful.getByID(hzq_rest+'gasmtrmeter',checkPlanDetail.meterId);
		checkMeter.lastCheckDate=new Date(moment().format("YYYY-MM-DD HH:mm:ss")+"-00:00");
		submitJson.sets.push({"txid":"23","body":checkMeter,"path":"/gasmtrmeter/","method":"PUT"});
	}
	checkPlanDetail.changeMeter=changeMeter;
	checkPlanDetail.modifiedBy=UserInfo.userId();
	checkPlanDetail.modifiedTime=new Date(moment().format("YYYY-MM-DD HH:mm:ss")+"-00:00");
	
	submitJson.sets.push({"txid":"20","body":checkPlanDetail,"path":"/gasmtrcheckplandetail/","method":"PUT"});
    ...
    if(retFlag){
        ...
        bootbox.alert("提交成功",function(){
    		window.location.replace("/customer/repair_order_examine.html");
        });
...
$("#meterNo").blur(function(){
    ...
    if(results&&results.length>0){
    	...
    	bootbox.alert('<center><h4>系统中无此新一次表信息，请对该表具进行信息补录</h4></center>');
    }
})
