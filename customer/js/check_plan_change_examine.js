...
var detailedInfoFormat =function(){
    return{
        f:function(val,row){
	        return "<a id='"+val+"' class='addmeter' data-rows='"+JSON.stringify(row)+"' data-toggle='modal' href='#allot'>查看详情</a>";
        }
    }
}()
...
var doBusi = function(step){
	var xwQuery = RQLBuilder.and([
	    RQLBuilder.equal("checkType","1"),RQLBuilder.equal("checkPlanId",checkPlan.checkPlanId)
	]).rql();	
	var resData = Restful.findNQ(hzq_rest+'gasmtrcheckplandetail?query='+xwQuery);
    checkPlan.modifiedTime = new Date(moment().format("YYYY-MM-DD HH:mm:ss")+"-00:00");
    checkPlan.modifiedBy = UserInfo.userId();
    checkPlan.checkCount = resData.length;
