...
$("#navtab li").on('click',function(){
    ...
    if($(this).find('a').text() == "表具档案 "){
        ...
        if(ctmmeter.length) {
            ...
            /*$.ajax({
             url:hzq_rest + 'gasctmmeter/?query={"ctmArchiveId":"'+archiveId+'"}',
             type:"GET",
             dateType:"json",
             success:function(data){
             console.log(data)
             if(data.length){
             console.log(data[0].meterId)
             $.ajax({
             url:hzq_rest + 'gasmtrmeter/?query={"meterId":"'+data[0].meterId+'"}',
             type:"GET",
             dateType:"json",
             success:function(data){
             console.log(data)
             ...
             console.log(data[0].meterModelId)
             console.log(data[0].factoryId);
             console.log(data[0].meterModelId);
             var queryCondion = RQLBuilder.and([
             RQLBuilder.equal("factoryId",data[0].factoryId),
             RQLBuilder.equal("meterModelId",data[0].meterModelId)
             ]).rql()
             var result = Restful.findNQ(hzq_rest + 'gasmtrmeterfactoryspec/?query='+queryCondion);
             console.log(result);
             if(result.length){
             var results = Restful.findNQ(hzq_rest + 'gasmtrmeterflow/'+result[0].meterModelId);
             console.log(results);
             $("span[name='flow']").html(results.ratingFlux);
             $("span[name='flowRange']").html(results.flowName);
             }
...
var bd={
    ...
    "wheres":"t1.customer_code is null  and t1.gas_kind = '1'",
    "page":false
};
$.ajax({
