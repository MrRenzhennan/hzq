...
var queryCondion = RQLBuilder.and([
    RQLBuilder.condition_fc("areaId","$in",JSON.stringify(areas)),
    RQLBuilder.equal("status","1"),
]).rql()


var AreaHelper1 = RefHelper.create({
    ref_url: 'gasbizarea?query='+queryCondion,
    ref_col: "areaId",
...
// changeMeterReason
            var bd = {
                "cols": "a.up_meter_no,a.down_meter_no,a.change_meter_reason,b.*,d.gas_type_id,d.money,d.trade_date",
                "froms": "gas_csr_work_bill_result a left join gas_csr_work_bill b on a.work_bill_id=b.work_bill_id inner join gas_chg_gas_detail e on e.customer_code = b.customer_code left join gas_act_gasfee_atl d on d.gasfee_atl_id = e.trade_id  ",
                "wheres": "b.bill_state='4' and to_char(b.complete_date,'yyyymm') = to_char(d.trade_date,'yyyymm') and b.bill_type in ('WB_CHANGEM','B_CHANGEM') and b.area_id in ("+areas.join()+") and e.status='1' and d.trade_date is not null order by d.trade_date desc,b.complete_date desc",
                "page": true,
                "limit": 50
            };
            var tradedateFormat= function(){
                return {
                    f:function(val,row){
                        console.log(row)
                        if(val){
            ...
            xw = XWATable.init(
                {
                    ...
                    findFilter: function () {
                        var whereinfo = '1=1';
                        if ($('#find_unit').val()) {
                            whereinfo += " and b.area_id='" + $('#find_unit').val() + "' ";
                        }
                        if ($('#find_customerCode').val()) {
                            whereinfo += " and b.customer_code='" + $('#find_customerCode').val() + "' ";
                        }
                        if ($('#find_customerName').val()) {
                            whereinfo += " and b.customer_name like '%" + $('#find_customerName').val() + "%' ";
                        }
                        if ($('#changeMeterReason').val()) {
                            whereinfo += " and a.change_meter_reason='" + $('#changeMeterReason').val() + "' ";
                        }
                        if ($('#find_gasTypeId').val() && !$('#find_gasTypeId1').val() && !$('#find_gasTypeId2').val()) {
                            whereinfo += " and d.gas_type_id  like '" + $('#find_gasTypeId').val() + "%' ";
                        } else if ($('#find_gasTypeId').val() && $('#find_gasTypeId1').val() && !$('#find_gasTypeId2').val()) {
                            whereinfo += " and d.gas_type_id like '" + $('#find_gasTypeId1').val() + "%' ";
                        } else if ($('#find_gasTypeId').val() && $('#find_gasTypeId1').val() && $('#find_gasTypeId2').val()) {
                            whereinfo += " and d.gas_type_id = '" + $('#find_gasTypeId2').val() + "' ";
                        }
                        if ($("#find_start_date").val() && $("#find_end_date").val()) {
                            whereinfo += " and to_char(b.complete_date,'yyyy-mm-dd')  between '" + $('#find_start_date').val() + "' and '" + $("#find_end_date").val() + "' "
                            whereinfo += " and to_char(d.trade_date,'yyyy-mm-dd')  between '" + $('#find_start_date').val() + "' and '" + $("#find_end_date").val() + "' ";
                        ...
                        var bd = {
                            "cols": "a.up_meter_no,a.down_meter_no,a.change_meter_reason,b.*,d.gas_type_id,d.money,d.trade_date",
                            "froms": "gas_csr_work_bill_result a left join gas_csr_work_bill b on a.work_bill_id=b.work_bill_id inner join gas_chg_gas_detail e on e.customer_code = b.customer_code left join gas_act_gasfee_atl d on d.gasfee_atl_id = e.trade_id  ",
                            "wheres": whereinfo + " and to_char(b.complete_date,'yyyymm') = to_char(d.trade_date,'yyyymm') and b.bill_state='4' and b.bill_type in ('WB_CHANGEM','B_CHANGEM') and b.area_id in ("+areas.join()+")and e.status='1' and d.trade_date is not null order by d.trade_date desc,b.complete_date desc",
                            "page": true,
                            "limit": 50
                        };
                        console.log(bd);
