...
$(".export-excel").attr("data-table", ".tabPrint");
		$(".export-excel").attr("data-filename", $("#find_date").val() + "&nbsp;&nbsp;表具额定流量在" + $("#find_date").val() + "月度销售（应收）明细");
		...
		jQuery(document).ready(function() {
			...
			$("#title").html($("#find_date").val() + "&nbsp;&nbsp;表具额定流量在" + $("#find_date").val() + "月度销售（应收）明细");
			//查询按钮时间
			...
			GasModMtr.meterFlowList({
				"cb":function(data){
					var inhtml = "<option value=''>全部</option>";
					var ohtml = "<option value=''>全部</option>";
					if(data){
						$.each(data,function(inx,row){
							inhtml +='<option value="'+row.meterFlowId+'">'+row.flowCode+":"+row.ratingFlux+'</option>';
							ohtml +='<option value="'+row.meterFlowId+'">'+row.flowCode+":"+row.flowName+'</option>';
						})
					}
					$('#find_meter_rating_flux').html(inhtml);
					$('#find_meter_flow_name').html(ohtml);
					
					$("#find_meter_rating_flux").val("").change();
					$("#find_meter_flow_name").val("").change();
				}
			});
			...
			/*GasModMtr.meterFlowList({
				"cb":function(data){
					...
					if(data){
						$.each(data,function(inx,row){
							inhtml +='<option value="'+row.meterTypeId+'">'+row.meterTypeCode+":"+row.flowName+'</option>'
						})
					}
					$('#find_meter_flow_name').html(inhtml);
					$("#find_meter_flow_name").val("").change();
				}
			});*/
...
for(var j=0;j<12;j++){//12个月
	if(j==0){
		...
		" ) then y.gas  else 0 end ) over(partition by nvl(d.rating_flux,'0')) as "+month_arr[(j+1)]+",";
	}else{
...
if($('#find_meter_rating_flux').val()){
	whereinfo +=" and y.meter_flow_id='"+$('#find_meter_rating_flux').val()+"' ";
}
if($('#find_meter_flow_name').val()){
	whereinfo +=" and y.meter_flow_id='"+$('#find_meter_flow_name').val()+"' ";
}
if($('#find_areaId').val()){
...
var bd={
	"cols":"distinct d.rating_flux,"+ //y.meter_flow_id,
	""+
	sqlcols+""+
	...
	" ) then y.gas else 0 end ) over(partition by nvl(d.rating_flux,'0')) as totaldata ",
	...
	//" left join gas_mtr_meter_flow my on my.meter_flow_id=y.meter_flow_id ",
						...
						if(results && results.rows && results.rows.length > 0) {
							...
							for(var i = 0; i < rows.length; i++) {
								...
								for(var s = 1; s <= 20; s++) {
									...
									var ratingflux = rows[i].ratingFlux;
									if(!ratingflux){
										ratingflux ='--';
									}
									if(s == 1) {
									...
									if(s == 2) {
										tablebodyhtml += '<td id="tbody_r' + ct + '_cc' + (s) + '" rowspan="" colspan="1">' + ratingflux + '</td>';
									} else
									if(s >= 8 && s <= 19) {
