...
function tabletoExcel() {
    ...
    // var caption_orig = table.getElementsByTagName("");
    // $(caption_orig).text(name);
    // var th_first_ele = table.getElementsByTagName("tr")[0];
    // th_first_ele.innerHTML="XXX的编号";

    // 下面的代码才是真正用来将html table导出Excel表格（从stackoverflow上看到的，修改了一点点，不会再有中文乱码问题了。）
    ...
    var template =
        ...
        '</head>' +
        '<body>' +
        '<table style="vnd.ms-excel.numberformat:@">' +
...
//        }
    var ctx = {worksheet: name || '',table1: table1.innerHTML ,table2: table2.innerHTML, table3: table3.innerHTML};
    window.location.href = uri + base64(format(template, ctx));

}
//导出Excel表格,mytable参数是待导出的table的id
function tabletoExcel1(mytable) {
	...
	var uri = 'data:application/vnd.ms-excel;base64,'
		...
		'<head><!--[if gte mso 9]><?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
		'<x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name>' +
		'<x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]-->' +
		'</head><body><table style="vnd.ms-excel.numberformat:@">{table}</table></body></html>';
