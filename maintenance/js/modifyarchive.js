...
$.each(GasModSys.areaHelper.getRawData(), function (idx, row) {
    $('#areaId').append('<option value="' + row.areaId + '" name="' + row.areaId + '">' + row.areaName + '</option>');
});
...
var querybookCondionss = RQLBuilder.and([
    RQLBuilder.equal("areaId",house.areaId),
    RQLBuilder.equal("bookType",house.customerKind),
]).rql()
console.log(0)
var bookcodes =Restful.findNQ(hzq_rest + "gasmrdbook/?query="+querybookCondionss)
console.log(bookcodes)
$.each(bookcodes,function(index,item){
    $("#bookId").append("<option value='"+item.bookId+"'>"+item.bookCode+"</option>");
    $("#bookId").val(house.bookId).trigger("change");
})
