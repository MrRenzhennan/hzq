...
$("#submit_btn").on('click',function(){
    ...
    var flow_def_id;
    if(customerKind=="1"){
    	busitype="长期不用气恢复用气申请(居民)";
    	flow_def_id="JMHFYQSQ";
    }else{
    	busitype="长期不用气恢复用气申请(非居民)";
    	flow_def_id="FJMHFYQSQ";
    }
    ...
    gascsrbusiregister.businessTypeId = "HFYQ";
...
function onCustomerSelected(customerCode){
    ...
    if(data.rows[0].customerState!="04"){
    	$('#save_btn').addClass('disabled');
    	bootbox.alert("<br><center><h4>只有长期不用气的用户才可申请长期不用气恢复用气</h4></center><br>");
    }else{
