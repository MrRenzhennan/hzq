                    ...
                    success: function (data) {
                        if(data.length>0){
                            ...
                            $.each(data[0], function(key,val) {
                                $("form[id='resultForm'] input[name='"+key+"']").val(val);
                                $("form[id='resultForm'] select[name='"+key+"']").val(val).trigger("change");
                            });
...
$("#meterNo").blur(function(){
    var _no = $("#meterNo").val();
	if(!_no){
		bootbox.alert('<center><h4>表编号不能为空</h4></center>');
		$("#save_btn").addClass("disabled");
    ...
    if(results&&results.length==1){
    	var data = results[0];
    	if(!data.meterKind){
    		bootbox.alert("<br><center>表编号为["+_no+"]的燃气表没有维护表具种类，请维护该表具信息</center></br>");
            $("#save_btn").addClass("disabled");
            return;
    	}else{
    		$("#save_btn").removeClass("disabled");
    	...
    	if(data.depositoryId&&data.depositoryId!=depository.depositoryId&&data.depositoryId!='2c91808300000012015d1285db070129'){
    		bootbox.alert("<br><center>表编号为["+_no+"]的燃气表不属于"+depository.depositoryName+"，请先做表具二级库入库</center></br>");
            $("#save_btn").addClass("disabled");
            return;
    	}else{
    		$("#save_btn").removeClass("disabled");
    	...
    	if(data.stockState&&data.stockState=='4'){
			bootbox.alert("<br><center>表编号为["+_no+"]的燃气表已上线，换表开栓的上线表状态应为在库，请核实表具信息</center></br>");
            $("#save_btn").addClass("disabled");
            return;
    	}else{
    		$("#save_btn").removeClass("disabled");
    	...
    	if(data.stockState&&data.stockState=='5'){
			bootbox.alert("<br><center>表编号为["+_no+"]的燃气表已下线，换表开栓的上线表状态应为在库，请核实表具信息</center></br>");
            $("#save_btn").addClass("disabled");
            return;
    	}else{
    		$("#save_btn").removeClass("disabled");
...
//  }
    ...
    if(!finishTime){
        bootbox.alert("<br><center>开栓完成时间</center></br>");
        $("#save_btn").removeClass("disabled");
        return;
    }else{
